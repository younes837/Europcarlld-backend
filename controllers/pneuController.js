...
const get_pneu_consomme = async (req, res) => {
  try {
    ...
    const columnMapping = {
      ...
      total_pneu_dotation: "(SELECT SUM(F470NBPNEUS) FROM [AlocproProd].[dbo].[PARC_CLIENT] PC_sub WHERE PC_sub.F050NOMPRE = PC.F050NOMPRE)",
      total_montant: "SUM(COALESCE(PP.F410MTHT, 0))",
      oldest_contract_date: "MIN(PC.DD)",
      consommation_moyenne: "CASE WHEN COUNT(DISTINCT PC.F091IMMA) > 0 THEN CAST(SUM(COALESCE(PP.F410QT, 0)) AS FLOAT) / COUNT(DISTINCT PC.F091IMMA) ELSE 0 END"
    };
